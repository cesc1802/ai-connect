...
import {
  WsProvidersResponse,
  type PutWsProvidersRequest,
} from '@/schemas/admin';

export class WsProvidersEtagMismatchError extends Error {
  readonly code = 'etag_mismatch';
  constructor() {
    super('Bindings changed');
    this.name = 'WsProvidersEtagMismatchError';
  }
}

export class WsProvidersNotInPoolError extends Error {
  readonly code = 'not_in_org_pool';
  constructor(readonly invalidIds: string[]) {
    super(`Not in org pool: ${invalidIds.join(', ')}`);
    this.name = 'WsProvidersNotInPoolError';
  }
}

export interface WsProvidersFetchResult {
  data: WsProvidersResponse;
  etag: string | null;
}
...
export async function getWsProviders(): Promise<WsProvidersFetchResult> {
  const res = await rawFetch('/admin/workspace/providers', {
    method: 'GET',
  ...
  const parsed = WsProvidersResponse.safeParse(json);
  if (!parsed.success) {
...
export async function putWsProviders(
  body: PutWsProvidersRequest,
  ifMatch: string | null,
): Promise<WsProvidersFetchResult> {
  ...
  if (res.status === 409) {
    throw new WsProvidersEtagMismatchError();
  }
  if (res.status === 400) {
    ...
    if (raw?.code === 'not_in_org_pool') {
      const ids = Array.isArray(raw.invalidIds)
        ...
        : [];
      throw new WsProvidersNotInPoolError(ids);
    }
